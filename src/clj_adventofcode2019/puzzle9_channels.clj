(ns clj-adventofcode2019.puzzle9-channels
  (:gen-class)
...
(defn- addition
  [config first-value second-value result-idx]
  (let [{program-code :program-code
         running-idx :running-idx
         argcount :argcount} config]
    (assoc config
...
(defn- put-in-pos
  [config first-value]
  ;(println (str "about to block on " (:input-channel config)))
  (let [{program-code :program-code
         running-idx :running-idx
         argcount :argcount
         input-channel :input-channel} config
        input (if (get config :read-from-stdin true)
...
(defn- print-pos
  [config first-value]
  (let [{running-idx :running-idx
         argcount :argcount
         output-channel :output-channel} config]
                                        ;(println first-value)
    (>!! output-channel first-value)
    (assoc config :running-idx (+ running-idx (inc argcount)) :output first-value :last-operation 4)))

(defn- jump-if-true
  [config first-value second-value]
  (let [{program-code :program-code
         running-idx :running-idx
         argcount :argcount} config]
...
(defn- less-than
  [config first-value second-value third-value]
  (let [{running-idx :running-idx
         argcount :argcount
         program-code :program-code} config]
...
(defn- adjust-relative-base
  [config first-value]
  (let [{relative-start :relative-start
         running-idx :running-idx
         argcount :argcount} config]
...
(defn- extract-operation-and-mask
  ...
  (let [operation (mod opcode 100)
    ...
    (if (not= (count modes-str) (:argcount (get configs-per-operation operation)))
      {:operation operation
       :mask (reverse
...
(defn calculate-output-for-state
  ...
  ([state]
   (let [program-code (:program-code state)
         ...
         new-state (assoc state :argcount (:argcount operation-configs))
     ...
     ;(go (>!! (:last-operation-channel state) (:operation operation-and-mask)))
     (if (= 99 (:operation operation-and-mask))
       (do
         (close! (:output-channel state))
         (cons state nil))
       (lazy-seq (cons
        state
        (calculate-output-for-state (apply (:fn operation-configs) (cons new-state values)))))))))

;{:program-code (load-code) :running-idx 0 :relative-start 0 :input 1 :read-from-stdin false :last-operation -1 :input-channel inchan :output-channel outchan}
...
(defn run-pt1
  ([]
   (run-pt1 {:program-code (load-code)
             ...
             :input-channel (chan)
             :output-channel (chan)
             ;:last-operation-channel (chan)
             }))
  ([state]
   (let [computer (thread (doall (calculate-output-for-state state)))]
     ;(>!! (:input-channel state) 1)
     ;(println (str "Result was " (<!! (:output-channel state))))
     ;(close! (:output-channel state))
                                        ;(close! (:input-channel state))
     computer
     )))
